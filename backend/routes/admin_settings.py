...
def create_admin_setting(payload: AdminSettingCreate):
    with get_db() as conn:
        return AdminSettingCRUD.create(conn, payload.model_dump())


@router.put("/{item_id}", response_model=AdminSettingOut)
def update_admin_setting(item_id: int, payload: AdminSettingUpdate):
    with get_db() as conn:
...
def delete_admin_setting(item_id: int):
    with get_db() as conn:
